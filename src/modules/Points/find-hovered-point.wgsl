...
fn vertexMain(input: VertexInput) -> VertexOutput {
  ...
  let uv = (input.pointIndices + vec2f(0.5)) / findHoveredPoint.pointsTextureSize;
  ...
  var normalizedPosition = 2.0 * point / findHoveredPoint.spaceSize - vec2f(1.0);
  ...
  let finalPosition = findHoveredPoint.transformationMatrix * vec4f(normalizedPosition, 1.0, 1.0);
  ...
  let pointScreenPosition = (finalPosition.xy + vec2f(1.0)) * findHoveredPoint.screenSize / 2.0;
  ...
  if (dist < pointRadius / findHoveredPoint.ratio) {
    ...
    output.rgba = vec4f(index, max(input.size, input.imageSize), pointPosition.xy);
    output.position = vec4f(-0.5, -0.5, 0.0, 1.0);
  }
