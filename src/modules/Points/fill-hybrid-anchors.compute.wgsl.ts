...
const PRIORITY_SCORE_SHIFT: u32 = 21u;
...
fn anchorPriority(pointIndex: u32, pointSize: f32, centerDistance: f32, sparse: bool) -> u32 {
  let tie = PRIORITY_TIE_MASK - min(pointIndex, PRIORITY_TIE_MASK);
  if (sparse) {
    return (0x7ffu << PRIORITY_SCORE_SHIFT) | tie;
  }
  let sizeScore = u32(clamp(pointSize * 16.0, 0.0, 63.0));
  let spreadScore = u32(clamp(centerDistance * 31.0, 0.0, 31.0));
  let score = (sizeScore << 5u) | spreadScore;
  return (score << PRIORITY_SCORE_SHIFT) | tie;
}
...
fn computeMain(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let priorityOffset = anchorUniforms.tileColumns * anchorUniforms.tileRows * anchorUniforms.anchorsPerTile;

  let centerDistance = distance(local, centroid);
  let priority = anchorPriority(i, sizes[i], centerDistance, sparse);
  _ = atomicMax(&anchorPriorities[priorityOffset + outIndex], priority);
}
`
}
